...
$userId = $_GET['id'] ?? $_SESSION['user_id']; // Pastikan user_id sudah ada di session

$_SESSION['username'];

try {
    // Query untuk mengambil data dari tabel yang sesuai dengan user_id
    $query = "SELECT id AS id_karya, category, title, content, media_url, created_at FROM works WHERE user_id = ?";
    $stmt = $db->prepare($query);
    $stmt->execute([$userId]);  // Bind parameter user_id dengan data dari session
    ...
    if (count($data) > 0) {
        foreach ($data as $row) {
            ...
            echo '<div class="ml-3">';
            // Tampilkan informasi lainnya, jika tidak ada kolom username, bisa gunakan kolom lain
            echo '<span class="font-weight-bold"> ' .$_SESSION ['username']. '</span><br>';
            // echo $_session['username']
            echo '<small class="text-muted">' . date("F j, Y, g:i a", strtotime($row['created_at'])) . '</small>';
            echo '</div>';
            echo '</div>';

            // Menampilkan media jika tersedia, jika tidak tampilkan placeholder
